...
namespace CP.Windows.IoT.Assets
{
    public class Font8x8
    {
        ...
        ************************************************************************/
        ...
        public class Character
        {
            ...
            #region Creation
            public Character(UInt64 data)
            {
                this.mBitmap = new byte[8];

                for (int row = 0; row < 8; row++)
                {
                    var rowData = (byte)((data & ((UInt64)255 << row * 8)) >> row * 8);
                    this.mBitmap[7 - row] = rowData;
                }
            }
            #endregion
            ...
            {
                return (this.mBitmap[row] & 1 << (8-col)) != 0;
            }

            public override string ToString()
            {
                var str = new StringBuilder();
                for (int row = 0; row < this.mBitmap.Length; row++)
                {
                    for (int col = 0; col < 8; col++)
                        str.Append(this.GetBit(row, col) ? "#" : "+");
                    str.AppendLine();
                }
                return str.ToString();
            }
        ...
        #region Variables
        private static Lazy<Dictionary<char, Character>> mCharacterSet = new Lazy<Dictionary<char, Character>>(LoadCharacters);
        #endregion
        ...
        public static Character GetCharacter(char character)
        {
            ...
            mCharacterSet.Value.TryGetValue(character, out result);
            return result;
        }
        
        
        private static Dictionary<char, Character> LoadCharacters()
        {
            var characters = new Dictionary<char, Character>();
            for (char i = (char)0; i < Font.Length; i++)
                characters[i] = new Character(Font[i]);
